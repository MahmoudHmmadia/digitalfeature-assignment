...
@Component({
  ...
  template: `<div
    class="flex items-center justify-between gap-3 border-t px-4 py-3 text-sm text-slate-600"
  >
    <span>{{ total() }} {{ t.text(itemLabel()) }}</span>
    <div class="flex items-center gap-2">
      ...
      ><span class="min-w-14 text-center">{{ page() }} / {{ pages() }}</span
      ><app-button
        variant="outline"
        [disabled]="page() >= pages()"
        (click)="pageChange.emit(page() + 1)"
