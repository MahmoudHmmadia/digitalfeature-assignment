...
export class RegisterService {
  ...
  readonly errors = computed((): Record<string, string | undefined> => {
    ...
    if (c.firstName.touched && c.firstName.errors) {
      if (c.firstName.errors['required']) result['firstName'] = 'First name is required';
      else if (c.firstName.errors['minlength']) result['firstName'] = 'At least 2 characters';
    }

    if (c.lastName.touched && c.lastName.errors) {
      if (c.lastName.errors['required']) result['lastName'] = 'Last name is required';
    }
